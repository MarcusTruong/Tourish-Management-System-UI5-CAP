sap.ui.define([
    ...
    return Controller.extend("tourishui.controller.supplier.SupplierInformation", {
        ...
        onGeneratePDF: function () {
            ...
            if (aSelectedItems.length === 0) {
                MessageBox.information("Please select at least one supplier to generate PDF");
                return;
            }

            console.log("Number of selected suppliers:", aSelectedItems.length);

            // Lấy Supplier IDs từ các items được select
            var aSupplierIDs = [];
            aSelectedItems.forEach(function (oItem) {
                ...
                aSupplierIDs.push({
                    id: oSupplier.ID,
                    name: oSupplier.SupplierName
                });
                console.log("Selected Supplier ID:", oSupplier.ID, "Name:", oSupplier.SupplierName);
            });

            // Mở các cửa sổ mới cho mỗi supplier
            this._openPDFWindows(aSupplierIDs);
        },

        _openPDFWindows: function (aSuppliers) {
            var sBaseURL = window.location.origin + window.location.pathname;

            // Mở cửa sổ mới cho mỗi supplier
            aSuppliers.forEach(function (oSupplier, index) {
                // Tạo URL với hash routing
                var sURL = sBaseURL + "#/supplier/pdf/" + oSupplier.id;

                console.log("Opening PDF window for:", oSupplier.name);
                console.log("URL:", sURL);

                // Mở cửa sổ mới với delay nhỏ để tránh popup blocker
                setTimeout(function () {
                ...
                }, index * 200); // Delay 100ms giữa các window để tránh bị block

            });

            MessageToast.show(aSuppliers.length + " PDF window(s) opened");
        }
